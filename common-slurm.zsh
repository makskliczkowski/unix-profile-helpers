...
# 1. Custom formatted queue for YOUR active jobs (Running = Green, Pending = Yellow, Completing = Red)
alias sq="squeue -u \$USER --format='%.8i %.9P %.18j %.8u %.2t %.10M %.6D %R' | awk '
  NR==1 {print \$0; next}
...
# 2. Custom formatted queue for ALL jobs in the cluster
